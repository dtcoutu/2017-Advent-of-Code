...
def input = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
def countValidPassphrases(String allPassphrases) {
	def validCount = 0
	allPassphrases.split("\\n").each {
		if (validPassphrase(it)) {
			validCount++
		}
	}
...
assert validPassphrase("aa bb cc dd aa") == false

assert validPassphrase("aa bb cc dd ee") == true

assert validPassphrase("aa bb cc dd aaa") == true
