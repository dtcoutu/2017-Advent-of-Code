case class Line(name: String, weight: Int, children: List[String] = List.empty)
case class Program(name: String, weight: Int, children: List[Program] = List.empty)

def parseInput(input: String): List[Line] = {
	def parseLine(line: String): Line = {
		line match {
			case InnerNode(name,weight,childBlock) => Line(name, weight.toInt, childBlock.split(", ").toList)
			case LeafNode(name,weight) => Line(name, weight.toInt)
		}
	}
...
val InnerNode = """(\w+) \((\d+)\)(?: -> ([a-z, ]+))""".r
val LeafNode = """(\w+) \((\d+)\)""".r

def buildTree(lines: List[Line]): Program = {
	def buildProgram(name: String): List[Program] = {
		lines.filter(_.name == name).map(line => {
			if (line.children.isEmpty) Program(line.name, line.weight)
			else Program(line.name, line.weight, line.children.flatMap(buildProgram))
		})
	}
	
	val allChildren = lines.flatMap(_.children)
	val root = lines.filterNot(i => allChildren contains i.name)(0)
	Program(root.name, root.weight, root.children.flatMap(buildProgram))
}

def findRoot(input: String): String = {
	val lines = parseInput(input)
	buildTree(lines).name
}

sealed trait WeightResult
case class NodeWeight(totalWeight: Int, ownWeight: Int) extends WeightResult
case class UnbalancedWeight(desiredWeight: Int) extends WeightResult

def findImbalance(input: String): Int = {
	val lines = parseInput(input)
	
	val tree = buildTree(lines)
	
	def sumWeight(program: Program): WeightResult = program match {
		case Program(_, weight, children) if children.isEmpty => NodeWeight(weight, weight)
		case Program(_, weight, children) => {
			val childrenResults = children.map(sumWeight)
			val foundUnbalanced = childrenResults.find {
				case UnbalancedWeight(_) => true
				case _ => false
			}
			foundUnbalanced match {
				case Some(unbalancedWeight) => unbalancedWeight
				case None => {
					val childrenWeights = childrenResults.collect { case NodeWeight(total, own) => NodeWeight(total, own) }
					val groupedWeights = childrenWeights.groupBy(_.totalWeight).values
					if (groupedWeights.size == 1) NodeWeight(childrenWeights.map(_.totalWeight).sum + weight, weight)
					else {
						val min = groupedWeights.minBy(_.size).head
						val max = groupedWeights.maxBy(_.size).head // why can't I just grab something from the two groups?
						UnbalancedWeight(min.ownWeight + max.totalWeight - min.totalWeight)
					}
				}
			}
		}
	...
	sumWeight(tree) match {
		case UnbalancedWeight(d) => d
		case _ => -1
	}
}

def testInput = """pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
assert (findImbalance(testInput) == 60)
...
println("root = " + findRoot(input))
println("imbalance = " + findImbalance(input))