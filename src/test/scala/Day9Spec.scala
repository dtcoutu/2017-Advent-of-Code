...
class Day9Spec extends FlatSpec with Matchers {
  ...
  "Garbage" should "handle single blocks of garbage" in {
    assert(Day9.input("<>") == List(Day9.Garbage()))
		assert(Day9.input("<random characters>") == List(Day9.Garbage()))
		assert(Day9.input("<<<<>") == List(Day9.Garbage()))
		assert(Day9.input("""<{o"i!a,<{i<a>""") == List(Day9.Garbage()))
  }
	...
	"Group" should "keep track of nested groups" in {
		...
		assert(Day9.input("{<{},{},{{}}>}") == List(Day9.Group(List(Day9.Garbage()))))

		assert(Day9.input("{<a>,<a>,<a>,<a>}") == List(Day9.Group(List(Day9.Garbage(), Day9.Garbage(), Day9.Garbage(), Day9.Garbage()))))
		
		assert(Day9.input("{{<a>},{<a>},{<a>},{<a>}}") == List(Day9.Group(List(Day9.Group(List(Day9.Garbage())),
																																					Day9.Group(List(Day9.Garbage())),
																																					Day9.Group(List(Day9.Garbage())),
																																					Day9.Group(List(Day9.Garbage()))))))
																																					
		assert(Day9.input("{{<!>},{<!>},{<!>},{<a>}}") == List(Day9.Group(List(Day9.Group(List(Day9.Garbage()))))))
	}
